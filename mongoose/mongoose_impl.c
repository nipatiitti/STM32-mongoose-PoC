...
// Add CORS headers to existing header definitions
#define CORS_HEADERS                                                    \
...
struct attribute s_speed_attributes[] = {
    {"led1", "int", NULL, offsetof(struct speed, led1), 0, false},
    {"led2", "int", NULL, offsetof(struct speed, led2), 0, false},
    {"led3", "int", NULL, offsetof(struct speed, led3), 0, false},
    {NULL, NULL, NULL, 0, 0, false}};

struct apihandler_data s_apihandler_speed = {
    {"speed", "data", false, 0, 0, 0UL},
    s_speed_attributes,
    sizeof(struct speed),
    (void (*)(void *))glue_get_speed,
    (void (*)(void *))glue_set_speed};

static struct apihandler *s_apihandlers[] = {
    (struct apihandler *)&s_apihandler_speed};
...
void mongoose_set_http_handlers(const char *name, ...) {
    ...
    if (h == NULL) {
        ...
        ((struct apihandler_data *)h)->getter =
            va_arg(ap, mongoose_data_func_t);
        ((struct apihandler_data *)h)->setter =
            va_arg(ap, mongoose_data_func_t);
        ...
        ((struct apihandler_ota *)h)->closer =
            va_arg(ap, mongoose_ota_closer_t);
...
static void upload_handler(struct mg_connection *c, int ev, void *ev_data) {
    ...
    if (us->marker == 'U' && ev == MG_EV_READ && us->expected > 0 &&
        c->recv.len > 0) {
        size_t alignment =
            512;  // Maximum flash write granularity (iMXRT, Pico)
...
size_t print_struct(void (*out)(char, void *), void *ptr, va_list *ap) {
    ...
    for (i = 0; a[i].name != NULL; i++) {
        ...
        len +=
            mg_xprintf(out, ptr, "%s%m:", i == 0 ? "" : ",", MG_ESC(a[i].name));
        if (strcmp(a[i].type, "int") == 0) {
            ...
            len +=
                mg_xprintf(out, ptr, "%s", *(bool *)attrptr ? "true" : "false");
...
size_t print_timeseries(void (*out)(char, void *), void *ptr, va_list *ap) {
    ...
    for (i = 0; i < count; i++) {
        ...
        len +=
            mg_xprintf(out, ptr, "%s[%lu,%g]", comma, timestamps[i], values[i]);
    }
    return len;
}
...
static void http_ev_handler(struct mg_connection *c, int ev, void *ev_data) {
    if (ev == MG_EV_HTTP_HDRS && c->data[0] == 0) {
        ...
        if (mg_match(hm->uri, mg_str("/api/#"), NULL) ||
            ...
            if ((u == NULL || (h != NULL && (u->level < h->read_level ||
                                             (hm->body.len > 0 &&
                                              u->level < h->write_level))))) {
...
static void handle_modbus_pdu(struct mg_connection *c, uint8_t *buf,
    ...
    if (len < 12) {
        ...
        // uint16_t tid = mg_ntohs(*(uint16_t *) &buf[0]);  // Transaction
        // ID uint16_t pid = mg_ntohs(*(uint16_t *) &buf[0]);  // Protocol
        // ID uint16_t len = mg_ntohs(*(uint16_t *) &buf[4]);  // PDU length
        // uint8_t uid = buf[6];                            // Unit
        // identifier
        if (func == 6) {  // write single holding register
            ...
            *(uint16_t *)&response[10] = mg_htons(value);
            ...
            if ((size_t)(num * 2 + 10) < sizeof(response)) {
                for (i = 0; i < num; i++) {
                    success = s_modbus_handlers.write_reg_fn(
                        (uint16_t)(start + i), mg_htons(data[i]));
            ...
            if ((size_t)(num * 2 + 9) < sizeof(response)) {
                ...
                for (i = 0; i < num; i++) {
                    success = s_modbus_handlers.read_reg_fn(
                        (uint16_t)(start + i), &val);
                    if (success == false) break;
                    data[i] = mg_htons(val);
                }
...
static void modbus_ev_handler(struct mg_connection *c, int ev, void *ev_data) {
    ...
    if (ev == MG_EV_READ) {
        ...
        handle_modbus_pdu(c, c->recv.buf,
                          len + 6);           // Parse PDU and call user
